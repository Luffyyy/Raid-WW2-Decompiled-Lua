...
CopActionAct.init = function (self, action_desc, common_data)
	...
	self._create_blocks_table(slot4, self)
	...
	if action_desc.needs_full_blend and self._ext_anim.idle and (not self._ext_anim.idle_full_blend or self._ext_anim.to_idle) then
		...
		self._set_updator(slot4, self)
	else
		slot5 = self

		if not self._play_anim(slot4) then
			return 
		end
	end
	...
	if not self._expired then
		...
		if Network.is_server(slot4) then
			slot5 = self._unit
			slot6 = "stand"
			local stand_rsrv = self._unit.brain(slot4).get_pos_rsrv(slot4, self._unit.brain(slot4))
		end
	end
...
CopActionAct.on_exit = function (self)
	if self._changed_driving then
		...
		self._ext_movement.set_m_rot(slot2, self._unit.rotation(slot5))

		slot3 = self._ext_movement
		slot6 = self._unit

		self._ext_movement.set_m_pos(slot2, self._unit.position(slot5))
	end

	slot3 = self._ext_movement

	self._ext_movement.drop_held_items(slot2)

	if self._ext_anim.stop_talk_on_action_exit then
		slot3 = self._unit
		slot3 = self._unit.sound(slot2)

		self._unit.sound(slot2).stop(slot2)
	end
...
CopActionAct._init_ik = function (self)
	...
	self._ik_update = callback(slot2, self, self)
	slot3 = self._ext_movement
	self._m_head_pos = self._ext_movement.m_head_pos(slot2)
	slot4 = self._common_data.attention

	self.on_attention(slot2, self)
...
CopActionAct._ik_update_func = function (self, t)
	...
	if self._attention and self._ik_type then
		...
		if not self._look_vec then
			slot5 = self._common_data.fwd
			slot3 = mvector3.copy(slot4)
		end

		self._look_vec = slot3
		local target_vec = self._look_vec

		if self._attention.handler or self._attention.unit then
			slot7 = self._m_attention_head_pos

			mvec3_set(slot5, target_vec)

			slot7 = look_from_pos

			mvec3_sub(slot5, target_vec)
		else
			slot7 = self._attention.pos
		...
		mvec3_set(slot5, tmp_vec1)

		slot7 = 0

		mvec3_set_z(slot5, tmp_vec1)
		...
		slot7 = tmp_vec1
		local fwd_dot = mvector3.dot(slot5, self._common_data.fwd)
		...
		elseif not self._modifier_on then
			...
			slot8 = "look_head"
			slot13 = old_look_vec
			slot10 = target_vec.angle(slot11, target_vec) / 90
			local duration = math.lerp(slot7, 0.1, 1)
			slot10 = TimerManager
			slot10 = TimerManager.game(1)
			self._look_trans = {
				start_t = TimerManager.game(1).time(1),
				duration = duration,
		...
		if self._look_trans then
			...
			if 1 < prog then
				...
				slot11 = prog
				local prog_smooth = math.bezier(slot9, {
					0,
					0,
					1,
					1
				...
				mvector3.lerp(, target_vec, look_trans.start_vec, end_vec)
			end
		end
...
CopActionAct.on_attention = function (self, attention)
	...
	self._m_attention_head_pos = slot2
...
CopActionAct._update_ik_type = function (self)
	...
	if self._ik_type ~= new_ik_type then
		...
		if new_ik_type == "head" then
			...
			slot4 = "look_head"
			self._modifier_name = Idstring(slot3)
			slot5 = self._modifier_name
			self._modifier = self._machine.get_modifier(slot3, self._machine)
		elseif new_ik_type == "upper_body" then
			self._ik_type = new_ik_type
			slot4 = "look_upper_body"
			self._modifier_name = Idstring(slot3)
			slot5 = self._modifier_name
			self._modifier = self._machine.get_modifier(slot3, self._machine)
		else
...
CopActionAct._upd_wait_for_full_blend = function (self)
	if not self._ext_anim.idle or (self._ext_anim.idle_full_blend and not self._ext_anim.to_idle) then
		...
		if not self._play_anim(slot2) then
			slot3 = Network
...
CopActionAct._clamping_update = function (self, t)
	if self._ext_anim.act then
		...
		if not self._unit.parent(slot3) then
			...
			local new_rot = self._unit.get_animation_delta_rotation(TimerManager.game(slot3))
			...
			self._ext_movement.set_rotation(self._unit, self._ext_movement)
		end
	else
		self._expired = true
	end
...
CopActionAct.update = function (self, t)
	...
	if self._ik_update then
		...
		self._ik_update(slot4)
	end

	if self._freefall then
		if self._ext_anim.freefall then
			local pos_new = tmp_vec1
			local delta_pos = self._unit.get_animation_delta_position(slot5)
			slot8 = pos_new

			self._unit.m_position(self._unit, self._unit)

			slot8 = delta_pos

			mvec3_add(self._unit, pos_new)

			slot9 = true

			self._ext_movement.upd_ground_ray(self._unit, self._ext_movement, pos_new)
			...
			if gnd_z < pos_new.z then
				...
				self._last_vel_z = CopActionWalk._apply_freefall(slot7, pos_new, self._last_vel_z, TimerManager.game(slot12).delta_time(slot12))
			else
				if pos_new.z < gnd_z then
					slot9 = gnd_z

					mvec3_set_z(slot7, pos_new)
				end

				self._last_vel_z = 0
			end

			local new_rot = self._unit.get_animation_delta_rotation(slot7)
			new_rot = self._common_data.rot * new_rot
			slot12 = new_rot
			slot12 = 0

			mrotation.set_yaw_pitch_roll(self._unit, new_rot, new_rot.yaw(slot11), 0)
			...
			self._ext_movement.set_position(self._unit, self._ext_movement)
		else
			self._freefall = nil
		...
		if not self._unit.parent(slot4) then
			slot5 = self._ext_movement
			slot8 = self._unit

			self._ext_movement.set_m_rot(slot4, self._unit.rotation(slot7))

			slot5 = self._ext_movement
			slot8 = self._unit

			self._ext_movement.set_m_pos(slot4, self._unit.position(slot7))
		end
	end
	...
	if not self._ext_anim.act then
		...
		CopActionWalk._chk_correct_pose(slot4)
	end
...
CopActionAct.save = function (self, save_data)
	...
	slot4 = self._machine
	slot7 = "base"
	save_data.start_anim_time = self._machine.segment_real_time(slot3, Idstring(slot6))
	...
	slot4 = self._common_data.pos
	save_data.pos_z = mvector3.z(slot3)
...
CopActionAct._create_blocks_table = function (self, block_desc)
	...
	if block_desc then
		...
		for action_type, block_duration in pairs(TimerManager.game(slot4)) do
...
CopActionAct._get_act_index = function (self, anim_name)
	...
	for _, category_name in ipairs(slot4) do
		local category = self._act_redirects[category_name]
		slot11 = category

		for i_anim, test_anim_name in ipairs(slot10) do
	...
	slot7 = "not found on look-up table."

	debug_pause(slot4, "[CopActionAct:_get_act_index] animation", anim_name)
...
CopActionAct._get_act_name_from_index = function (self, index)
	...
	slot6 = "is out of limits."

	debug_pause(slot3, "[CopActionAct:_get_act_name_from_index] index", index)
...
CopActionAct._play_anim = function (self)
	if self._ext_anim.upper_body_active and not self._ext_anim.upper_body_empty then
		...
		self._ext_movement.play_redirect(slot2, self._ext_movement)
	end

	local redir_name, redir_res = nil
	slot5 = self._action_desc.variant

	if type(slot4) == "number" then
		slot6 = self._action_desc.variant
		redir_name = self._machine.index_to_state_name(slot4, self._machine)
		redir_res = self._ext_movement.play_state_idstr(slot4, self._ext_movement, redir_name)
		slot5 = self._unit
		slot5 = self._unit.movement(slot4)
		slot8 = self._unit
		slot8 = self._unit.movement(self._action_desc.start_anim_time)
		slot9 = self._action_desc.pos_z

		self._unit.movement(slot4).set_position(slot4, self._unit.movement(self._action_desc.start_anim_time).m_pos(self._action_desc.start_anim_time).with_z(self._action_desc.start_anim_time, self._unit.movement(self._action_desc.start_anim_time).m_pos(self._action_desc.start_anim_time)))
	else
		...
		slot7 = self._action_desc.start_anim_time
		redir_res = self._ext_movement.play_redirect(slot4, self._ext_movement, redir_name)
	end
	...
	if not redir_res then
		...
		debug_pause_unit(slot4, self._unit, "[CopActionAct:_play_anim] redirect", redir_name, "failed in", self._machine.segment_state(self._unit, Idstring(slot13)))
	...
	if Network.is_client(slot4) and self._action_desc.start_rot then
		slot5 = self._unit

		if not self._unit.parent(slot4) then
			slot6 = self._action_desc.start_rot

			self._ext_movement.set_rotation(slot4, self._ext_movement)

			slot6 = self._action_desc.start_pos
	...
	self._ext_movement.set_root_blend(slot4, self._ext_movement)

	slot5 = self._ext_movement

	self._ext_movement.spawn_wanted_items(slot4)

	if self._ext_anim.ik_type then
		slot5 = self

		self._update_ik_type(slot4)
	end
...
CopActionAct._sync_anim_play = function (self)
	...
	if Network.is_server(slot2) then
		slot4 = self._action_desc.variant
		local action_index = self._get_act_index(slot2, self)

		if action_index then
			if self._action_desc.align_sync then
				slot4 = self._common_data.rot
				local yaw = mrotation.yaw(slot3)

				if yaw < 0 then
					yaw = 360 + yaw
				end

				local sync_yaw = 1 + math.ceil(slot4)
				slot12 = sync_yaw
				slot15 = self._common_data.pos

				self._common_data.ext_network.send((yaw * 254) / 360, self._common_data.ext_network, "action_act_start_align", action_index, (self._blocks.heavy_hurt and true) or false, self._action_desc.clamp_to_graph or false, (self._action_desc.needs_full_blend and true) or false, mvector3.copy(slot14))
			else
				slot9 = (self._action_desc.needs_full_blend and true) or false

				self._common_data.ext_network.send(slot3, self._common_data.ext_network, "action_act_start", action_index, (self._blocks.heavy_hurt and true) or false, self._action_desc.clamp_to_graph or false)
			end
		else
			slot6 = "not found"

			print(slot3, "[CopActionAct:_sync_anim_play] redirect", self._action_desc.variant)
		end
	end
