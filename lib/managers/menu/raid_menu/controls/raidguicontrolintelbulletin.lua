if not RaidGUIControlIntelBulletin then
...
RaidGUIControlIntelBulletin.set_data = function (self, item_value)
	...
	slot10 = self._title
	text_top_coord = self._title.bottom(self)
	slot10 = self._title
	content_panel_height = content_panel_height + self._title.h(self)
	...
	if self._data.texture and self._data.texture_rect then
		...
		slot10 = self._title
		text_top_coord = self._title.bottom(slot9)
	end
